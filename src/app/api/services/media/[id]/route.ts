...
export async function GET(_req: NextRequest, { params }: RouteContext) {
  try {
    ...
    const { data, error } = await supabase
      .from("service_media")
      .select("id, media_type, file_data, file_name")
      .eq("id", mediaId)
      .single();
    ...
    const contentType = guessServiceMediaContentType(
      (data as { media_type: string }).media_type === "video" ? "video" : "image",
    ...
    console.error("Service media API Error:", error);
