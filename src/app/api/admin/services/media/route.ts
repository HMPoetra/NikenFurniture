...
import { encodeBytea } from "@/lib/service-media";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const mediaTypeRaw = String(formData.get("mediaType") || "image").toLowerCase();
    ...
    if (file.size > maxBytes) {
      return NextResponse.json(
        { success: false, message: mediaType === "video" ? "Ukuran video maksimal 50MB." : "Ukuran gambar maksimal 5MB." },
        { status: 400 },
      );
    }
    ...
    const { data, error } = await supabase
      .from("service_media")
      .insert(payload)
      .select("id, service_id, media_type, file_name, urutan")
      .single();
